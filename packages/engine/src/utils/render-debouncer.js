...
function debounce(stateMutator) {
  ...
  const averageInterMutateInterval =
    (now - startTime) / pendingStateMutators.length;
  const timerInterval =
    averageInterMutateInterval < 32 ? averageInterMutateInterval * 3 + 4 : 100; // Never wait more than 100 ms
  timer = setTimeout(() => {
