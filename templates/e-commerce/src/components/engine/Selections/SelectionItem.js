  ...
  return (
    <li className="gx-media">
      <div className="gx-user-thumb gx-mr-3">
        {qSelectedCount > 0 ? <span className="gx-badge gx-badge-danger gx-text-white gx-rounded-circle">{qSelectedCount}</span> : null}
      </div>
      <div className="gx-media-body">
        <div className="gx-flex-row gx-justify-content-between gx-align-items-center">
          <h5 className="gx-text-capitalize gx-user-name gx-mb-0"><span className="gx-link">{qField}</span></h5>
        </div>
        <p className="gx-fs-sm">{qSelected}</p>
        <span 
          onClick={() => clear(qField)}
          className="gx-btn gx-btn-sm gx-top2 gx-text-muted">
          <i className="icon icon-trash gx-pr-2"/>Clear</span>
      </div>
    </li>
  );
};
